...
public class est_hospital {
    ...
    public void inserirInicio(int id, String sexo, int idade, boolean gestante,
        boolean lactante, boolean necessidadeEspecial,
        int prioridade) {
        logi logi =
            new logi(id, sexo, idade, gestante, lactante, necessidadeEspecial,
                prioridade);
        logi.proximo = inicio;
        inicio = logi;
        tamanho++;
    ...
    public void inserirFim(int id, String sexo, int idade, boolean gestante,
        boolean lactante, boolean necessidadeEspecial,
        int prioridade) {
        logi logi =
            new logi(id, sexo, idade, gestante, lactante, necessidadeEspecial,
    ...
    public void inserirIndice(int indice, int id, String sexo, int idade,
        boolean gestante, boolean lactante,
        boolean necessidadeEspecial, int prioridade) {
        if (indice <= 0) {
            inserirInicio(id, sexo, idade, gestante, lactante,
                necessidadeEspecial, prioridade);
        } else if (indice >= tamanho) {
            inserirFim(id, sexo, idade, gestante, lactante, necessidadeEspecial,
                prioridade);
        } else {
            logi local = inicio;
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 10; i++) {
            for (int h = 0; h < 5; h++) {
                ...
                naoAtendidas.inserirInicio(id, sexo, idade, false, false, false,
                    0);
                id++;
            }
            for (int h = 0; h < 3; h++) {
            ...
            for (int f = 0; f <= i; f++) {
                ...
                atendidas.inserirFim(atendido.id, atendido.sexo, atendido.idade,
                    atendido.gestante, atendido.lactante,
                    atendido.necessidadeEspecial,
                    atendido.prioridade);
            }
        }

        System.out.println("Pessoas atendidas: ==========" + atendidas);
        System.out.println("Pessoas nC#o atendidas: ==============" +
            naoAtendidas);
    }
        ...
        if (naoAtendidas.tamanho > 1) {
            for (int k = 1; k < naoAtendidas.tamanho; k++) {
                if (naoAtendidas.selecionarIndice(k).prioridade >
                    ...
                    naoAtendidas.inserirIndice(k - 1, aux2.id, aux2.sexo,
                        aux2.idade, aux2.gestante,
                        aux2.lactante,
                        aux2.necessidadeEspecial,
                        aux2.prioridade);
                    naoAtendidas.inserirIndice(k, aux.id, aux.sexo, aux.idade,
                        aux.gestante, aux.lactante,
                        aux.necessidadeEspecial,
                        aux.prioridade);
